...
/**
 * A class that implements easily callable logging features to a local file.
 * The logs in this class are written as plaintext files and stored in the
 * same directory as this JVM assumes to run in. A different path may be specified,
 ...
 * the directory of this class, maybe with a subdirectory Logs.
 ...
 * Starting this with elevated privileges is therefore <b>strongly</b> recommended.
 */
public class ErrorLogger {
     ...
     * the same place as the executed jar or class file.
     * The log will be signed with the date and time from the system.
     * @throws IOException 
     */
    ...
    /**
     * Private constructor that allows specifying a custom {@link java.time.LocalDateTime LocalDateTime} and a different
     * {@link java.nio.file.Path Path} to initialize with.
     * Appends the default filename to the Path if only a directory was given.
     ...
     * given, the directories will be created as specified using {@link java.nio.file.Files#createDirectories(java.nio.file.Path, java.nio.file.attribute.FileAttribute...) Files.createDirectories(path/pathParts)},
     * as long as a valid path can be created from it.
     * @param pathParts A variable amount of Strings for creating subdirectories.
